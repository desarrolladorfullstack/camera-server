...
# TASKS:
# 1. read files from logs {folders: (bk/dualcam/ logs/)}
...
ROOT="/home/marcel/"
SCRIPT_FILE="${ROOT}scripts/backup_log.sh"
LOCATIONS=("bk/dualcam/" "logs/")
if [[ "$1" != "" ]]
then
    {
    ...
    do
        {
            ...
            do
                echo "date_from: $date_from"
                {
                    ...
                    {
                        mv *${date_from}*".error" "${directory}/";
                        mv *${date_from}*".errors" "${directory}/";
                    } || { 
                        echo "could not move error files to $directory"
                    }
                    {
                        mv *${date_from}*".log" "${directory}/"; 
                        mv *${directory}*".log" "${directory}/"; 
                    } || { 
                        echo "could not move log files to $directory"
                    }
                    {
